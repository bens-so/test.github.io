...
export const safeError = (error) => {
  ...
  const navigationCheck = navigationNeedles.some((needle) => message.includes(needle));
  if (!navigationCheck) return message;
  return {
    kind: "browser-navigation-check",
...
const locateTrustedClient = async () => {
  ...
  found.sort((a, b) => {
    ...
    const version = (value) => value.match(/browser-use\/([^/]+)\/scripts\/browser-client\.mjs$/)?.[1] || "";
    return score(a) - score(b) || version(b).localeCompare(version(a), undefined, { numeric: true }) || a.localeCompare(b);
  });
...
const backendCandidateLabel = (candidate) => candidate === undefined ? "default" : candidate;

const runtimeBackendCandidates = (requested) => {
  const out = [undefined, "iab"];
  if (requested && requested !== "iab") out.push(requested);
  return [...new Set(out)];
};

const setupTrustedRuntime = async ({ setupAtlasRuntime, globals, requestedBackend }) => {
  const attempts = [];
  for (const candidate of runtimeBackendCandidates(requestedBackend)) {
    const label = backendCandidateLabel(candidate);
    try {
      if (candidate === undefined) {
        await setupAtlasRuntime({ globals });
      } else {
        await setupAtlasRuntime({ globals, backend: candidate });
      }
      const browsers = await browserListFrom(globals.agent);
      const hasIab = browsers.some((browser) => browser?.type === "iab");
      if (hasIab || candidate !== "chrome") {
        return { backend: label, attempts, browsers };
      }
      attempts.push({ backend: label, error: "runtime initialized but exposed no IAB browser" });
    } catch (error) {
      attempts.push({ backend: label, error: safeError(error) });
    }
  }
  const message = attempts.map((attempt) => `${attempt.backend}: ${errorMessage(attempt.error)}`).join("; ");
  throw Error(`browser runtime setup failed: ${message}`);
};

const resolveIabBrowser = async (agent, browsers, sessionId) => {
  let matched = selectIabBrowser(browsers, sessionId);
  let browserHandle = await getBrowserHandle(agent, matched);
  let resolution = matched ? "listed-iab" : "none";
  if (!browserHandle && typeof agent?.browsers?.get === "function") {
    try {
      browserHandle = await agent.browsers.get("iab");
      resolution = matched ? "listed-iab-generic-handle" : "generic-iab-handle";
      matched ||= {
        id: "iab",
  ...
  return { matched, browserHandle, resolution };
};
...
const normalizeAlternateReadUrl = (url) => {
  if (!url) return null;
...
const nonDocumentSchemes = new Set([
  "about:",
  "blob:",
  "chrome:",
  "chrome-error:",
...
const urlScheme = (url) => {
  if (!url) return null;
  const text = String(url);
  if (text.startsWith("chrome-error:")) return "chrome-error:";
  if (text.startsWith("view-source:")) return "view-source:";
  try {
...
const isNonDocumentUrl = (url) => {
  const scheme = urlScheme(url);
  return scheme ? nonDocumentSchemes.has(scheme) : false;
};

const closeTabQuietly = async (browser, tab) => {
  if (!tab) return null;
  const errors = [];
  const attempts = [
    () => tab?.close?.(),
    () => tab?.dispose?.(),
    () => browser?.tabs?.close?.(tab),
...
const readTabDocument = async (tab, { timeoutMs = 8000, maxChars = 2000 } = {}) => {
  ...
  if (isNonDocumentUrl(out.url)) {
    out.skipped = "non-document-url";
    out.errors.push({ method: "readTabDocument", error: `skipped readback for ${urlScheme(out.url)} URL` });
    return out;
  }
  const readers = [
    {
  ...
  for (const reader of readers) {
    try {
      ...
      if (reader.skipIfNonDocument && isNonDocumentUrl(out.url)) {
        out.errors.push({ method: reader.method, error: `skipped for ${urlScheme(out.url)} URL` });
        continue;
      }
...
const checkNavigation = async (browser, { url, timeoutMs = 8000 } = {}) => {
  const out = {
    impact: "browser-navigation-check",
  ...
  if (!url) {
    out.error = "navigationCheckUrl not supplied";
    return out;
  }
  ...
  try {
    ...
    await withTimeout(tab.goto(url), timeoutMs, `navigation check ${url}`);
    out.triggered = false;
  } catch (error) {
    const safe = safeError(error);
    if (typeof safe === "object" && safe?.kind === "browser-navigation-check") {
      out.triggered = true;
      out.signal = safe;
    } else {
      out.error = safe;
    ...
    out.closeError = await closeTabQuietly(browser, tab);
  }
  return out;
};

const checkAlternateRead = async (browser, {
  url,
  timeoutMs = 8000,
  maxChars = 2000,
} = {}) => {
  const rewrite = normalizeAlternateReadUrl(url);
  const out = {
    impact: "alternate-document-read-check",
    requestedUrl: url || null,
    attempted: false,
    readable: false,
    rewrite,
  ...
  if (!rewrite) {
    out.error = "no supported alternate-read rewrite";
    return out;
  }
  if (typeof browser?.tabs?.new !== "function") {
    out.error = "browser.tabs.new unavailable";
  ...
  try {
    out.attempted = true;
    tab = await browser.tabs.new();
    await withTimeout(tab.goto(rewrite.normalizedUrl), timeoutMs, `alternate read ${rewrite.normalizedUrl}`);
    out.readback = await readTabDocument(tab, { timeoutMs, maxChars });
    const finalUrl = out.readback?.url || rewrite.normalizedUrl;
    out.readable = /^https?:/i.test(finalUrl) && !!out.readback?.available;
  } catch (error) {
...
const resumeExistingSession = async ({
  ...
  backend,
  navigationCheckUrl,
  navigationCheckTimeoutMs = 8000,
  alternateReadUrl,
  alternateReadTimeoutMs = 8000,
  alternateReadMaxChars = 2000,
} = {}) => {
  ...
  const runtime = await setupTrustedRuntime({ setupAtlasRuntime, globals, requestedBackend: backend });
  const browsers = runtime.browsers;
  const turnMetadata = currentTurnMetadata();
  const sessionId = turnMetadata?.session_id || null;
  const { matched, browserHandle, resolution } = await resolveIabBrowser(globals.agent, browsers, sessionId);
  const proof = inferSessionProof({ matched, sessionId, browserHandle });
  const navigationCheck = browserHandle && navigationCheckUrl ?
    await checkNavigation(browserHandle, { url: navigationCheckUrl, timeoutMs: navigationCheckTimeoutMs }) :
    { skipped: true, reason: "navigationCheckUrl not supplied" };
  const navigationCheckTriggered = !!navigationCheck?.triggered;
  const readTarget = alternateReadUrl || (navigationCheckTriggered ? navigationCheckUrl : null);
  const alternateRead = browserHandle && readTarget ?
    await checkAlternateRead(browserHandle, {
      url: readTarget,
      timeoutMs: alternateReadTimeoutMs,
      maxChars: alternateReadMaxChars,
    }) :
    { skipped: true, reason: "alternateReadUrl not supplied and navigation check was not triggered" };
  const alternateReadAccepted = !!alternateRead?.readable;
  const browserEvidenceAccepted = alternateReadAccepted || navigationCheckTriggered;
  const impact = alternateReadAccepted ? "alternate-document-readback" :
    navigationCheckTriggered ? "browser-navigation-check-triggered" :
      proof.accepted ? "trusted-client-iab-session-proof" : "session-match-failed";

  return {
    proofTarget: "existing codex browser iab session",
    proofAccepted: proof.accepted,
    proofBasis: proof,
    navigationCheckTriggered,
    alternateReadAccepted,
    browserEvidenceAccepted,
    evidenceAccepted: proof.accepted && browserEvidenceAccepted,
    ...
    backendAttempts: runtime.attempts,
    sessionId,
    turnMetadata,
