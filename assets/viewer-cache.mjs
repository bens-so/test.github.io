...
const resumeExistingSession = async ({
  ...
  const impact = policyAccepted ? "browser-policy-rejection-captured" :
    sessionMatched ? "policy-probe-not-captured" : "session-match-failed";

  return {
    proofTarget: "existing codex browser iab session",
    proofAccepted: sessionMatched,
    accepted: sessionMatched && policyAccepted,
    impact,
