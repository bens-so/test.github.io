...
const openSameSessionSeedTab = async (browser, { url, timeoutMs = 8000, snapshotChars = 1200 } = {}) => {
  const out = {
    impact: "same-session-seeded-tab",
  ...
  if (!url) {
    out.skipped = true;
    out.error = "sameSessionSeedUrl not supplied";
    return out;
  }
  if (typeof browser?.tabs?.new !== "function") {
  ...
  try {
    out.beforeTabs = typeof browser?.tabs?.list === "function" ?
      (await browser.tabs.list()).map(compactTab) : null;
  } catch (error) {
    out.beforeTabs = [{ error: `tabs.list/${errorMessage(error)}` }];
  }
  ...
  try {
    tab = await browser.tabs.new();
    out.opened = true;
    out.tabId = tab?.id ?? null;
    await withTimeout(tab.goto(url), timeoutMs, `same-session seed ${url}`);
    out.navigated = true;
  } catch (error) {
    out.error = errorMessage(error);
  }

  if (tab) out.readback = await readTab(tab, { snapshotChars });

  try {
    out.afterTabs = typeof browser?.tabs?.list === "function" ?
      (await browser.tabs.list()).map(compactTab) : null;
  } catch (error) {
    out.afterTabs = [{ error: `tabs.list/${errorMessage(error)}` }];
  }
  return out;
};
...
const resumeExistingSession = async ({
  ...
  let crossTabReadback = browserHandle ?
  ...
  const preSeedCrossTabAccepted = crossTabReadback?.readable?.length > 0;
  const sameSessionSeed = browserHandle ?
    await openSameSessionSeedTab(browserHandle, {
      url: sameSessionSeedUrl,
      timeoutMs: sameSessionSeedTimeoutMs,
    }) :
    { error: "matching browser handle unavailable" };
  if (browserHandle && sameSessionSeed?.opened && !preSeedCrossTabAccepted) {
    crossTabReadback = await readNonDiagnosticTabs(browserHandle, { preferredBase, maxTabs: maxCrossTabs });
  }
  ...
  const sameSessionSeedAccepted = !!sameSessionSeed?.readback?.available;
  const seededCrossTabAccepted = !preSeedCrossTabAccepted && crossTabAccepted && sameSessionSeedAccepted;
  const impact = crossSessionAccepted ? "cross-session-readback" :
    seededCrossTabAccepted ? "seeded-same-session-cross-tab-readback" :
      crossTabAccepted ? "cross-tab-readback" :
        sameSessionSeedAccepted ? "same-session-seeded-tab-opened" :
